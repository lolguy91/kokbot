...
function replacedefaultmessages(text, o = {}) {
	...
	return String(text)
		.replace(
			/%{timeleft}%/gi,
			options && options.timeLeft
				? options.timeLeft.toFixed(1)
				: "%{timeleft}%"
		)
		.replace(
			/%{commandname}%/gi,
		...
		.replace(
			...
			options && options.command && options.command.memberpermissions
				? options.command.memberpermissions
						.map((v) => `\`${v}\``)
						.join(",")
...
function onCoolDown(message, command) {
	...
	if (timestamps.has(message.member.id)) {
		...
		const expirationTime =
			timestamps.get(message.member.id) + cooldownAmount; //get the amount of time he needs to wait until he can run the cmd again
...
function change_status(client) {
	try {
		client.user.setActivity(
			`/ai image |${client.guilds.cache.size} Guilds | ${Math.ceil(
